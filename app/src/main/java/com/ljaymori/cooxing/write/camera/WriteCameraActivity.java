...
public class WriteCameraActivity extends ActionBarActivity implements View.OnClickListener, SurfaceHolder.Callback, Camera.PictureCallback {

    public static final String TAG = WriteCameraActivity.class.getSimpleName();
    ...
    private WriteCameraAdapter mAdapter;
    private TextView tvChange, tvFlash;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        viewLoading = findViewById(R.id.view_loading_write_camera);
        ...
        preview = (SquareCameraPreview) findViewById(R.id.surfaceView_write_camera);
        preview.getHolder().addCallback(this);

        tvChange = (TextView) findViewById(R.id.text_change_angle_write_camera);
        tvFlash = (TextView) findViewById(R.id.text_flash_mode_write_camera);
        viewTakePicture = findViewById(R.id.view_take_picture_write_camera);

        tvChange.setOnClickListener(this);
        tvFlash.setOnClickListener(this);
        viewTakePicture.setOnClickListener(this);


        recyclerView = (RecyclerView) findViewById(R.id.recyclerView_write_camera);
        recyclerView.setLayoutManager(new LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false));

        mAdapter = new WriteCameraAdapter(this);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.text_change_angle_write_camera: {
            ...
            case R.id.view_take_picture_write_camera: {
                if(addedCount + mAdapter.getItemCount() < 10){
                    showLoadingDialog();
                    takePicture();

                } else {
                    Toast.makeText(WriteCameraActivity.this, getString(R.string.write_message_add_image_limit), Toast.LENGTH_SHORT).show();
                }
                break;
            }
            case R.id.text_flash_mode_write_camera: {
                if (flashMode.equalsIgnoreCase(Camera.Parameters.FLASH_MODE_AUTO)) {
                    flashMode = Camera.Parameters.FLASH_MODE_ON;
                    tvFlash.setBackgroundResource(R.drawable.ic_flash_on);

                } else if (flashMode.equalsIgnoreCase(Camera.Parameters.FLASH_MODE_ON)) {
                    flashMode = Camera.Parameters.FLASH_MODE_OFF;
                    tvFlash.setBackgroundResource(R.drawable.ic_flash_off);

                } else if (flashMode.equalsIgnoreCase(Camera.Parameters.FLASH_MODE_OFF)) {
                    flashMode = Camera.Parameters.FLASH_MODE_AUTO;
                    tvFlash.setBackgroundResource(R.drawable.ic_flash_auto);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.menu_write_camera, menu);
        return true;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        } else if (id == R.id.action_post_write_camera) {
            if (mAdapter.getItemCount() > 0) {
                Intent data = new Intent();
                data.putExtra(WriteDescFragment.FILE_PATHS, mAdapter.getItems());
                setResult(Activity.RESULT_OK, data);
                finish();

            } else {
                Toast.makeText(this, getString(R.string.write_message_insufficient_file_path), Toast.LENGTH_SHORT).show();
            }
            return true;
        }
    ...
    private void checkFilesAndExit() {
        if (mAdapter.getItemCount() > 0) {

            AlertDialog.Builder builder = new AlertDialog.Builder(this);

            builder.setTitle(getString(R.string.write_camera_dialog_title))
                    .setMessage(getString(R.string.write_camera_dialog_message))
                    .setCancelable(true)
                    .setPositiveButton(getString(R.string.ok), new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            for (String path : mAdapter.getItems()) {
                                removeFile(path);
                                finish();
                            }
                        }
...
//        ((CameraActivity) getActivity()).returnPhotoUri(photoUri);
    ...
    private void showLoadingDialog() {
        if (viewLoading.getVisibility() == View.GONE) {
